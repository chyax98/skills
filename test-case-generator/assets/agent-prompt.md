...
**如果发现乱码**：
1. 查看 grep 输出，精确定位哪一行有 `�` 字符
2. ❌ **立即停止**，不要继续下一个模块
3. 🔄 **重新生成该文件**，确保正确的 UTF-8 编码
4. ✅ 重新生成后**再次运行 grep 验证**

**输出示例**：
```bash
# 有乱码的情况
$ grep -n '�' cases/用户登录.jsonl
3:{"id":"M01-003","name":"验证用户�功登录","module_name":"用户登录"...
...
**正确做法**：
- ✅ JSON 中的中文直接写入即可，无需转义
- ✅ 使用 Write 工具时确保内容是有效的 UTF-8 字符串
